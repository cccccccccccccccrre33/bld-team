...
ГОТОВАЯ ТОЧКА ВХОДА ДЛЯ ПРОМТА 2 (context/idea_to_ecosystem_pipeline.md):
этот фильтр сейчас жёстко привязан к FELLOW_BUILDERS — задача Промта 2
снять эту привязку, чтобы filter_proposal() принимал предложение от
любого источника, не только Fellow. Материал для этого уже готов и
ждёт здесь: workflows/product_backlog.py::pull_concept_candidates()
отдаёт открытые записи scope='крупное' от ЛЮБОГО источника (field_immersion,
individual/squad initiative, ручная подача через main_submit_idea.py) —
именно они кандидаты на прогон через этот тройной фильтр после Промта 2,
без необходимости придумывать новый способ находить кандидатов.
