...
# Максимум активных задач ОДНОВРЕМЕННО В ОДНОМ SCOPE (in_progress).
# РАНЬШЕ было жёстко зашито в 4, потом поднято до 12 — но даже 12 было
# ОДНИМ счётчиком на буквально ВСЮ компанию: обсуждение отрядов BLD,
# индивидуальная инициатива, Company Pulse И любой новый продукт из
# register_project() (workflows/big_projects.py) — все делили одни и
# те же 12 слотов. Реальный технический риск, который лимит призван
# ограничивать — это параллельный git push/merge МНОГИХ веток В ОДИН
# И ТОТ ЖЕ main (конфликты) — а это риск ПО РЕПОЗИТОРИЮ, не риск
# "слишком много людей думают одновременно" (мысль/обсуждение бесплатно
# параллелится, конфликтует только реальный git-мердж). По прямому
# запросу Валика ("не должно быть так, что 14 думают, 1 реализует, а
# 600 просто ничего не делают — они должны работать параллельно, без
# очереди, командами под конкретную идею") лимит теперь СКОПИРОВАН по
# репозиторию/продукту (см. параметр scope в get_active_tasks() ниже),
# а не общий на всю компанию: у BLD (repo не указан явно — легаси-путь,
# см. add_task()) — своя ёмкость, у каждого нового продукта из
# register_project(..., repo=...) — СВОЯ НЕЗАВИСИМАЯ ёмкость, они друг
# друга не блокируют и не делят слоты. Само число (сколько параллельных
# веток безопасно мержить в ОДИН репозиторий) по-прежнему настраивается
# через MAX_CONCURRENT_TASKS (без правки кода), с дефолтом 12 — этот
# дефолт применяется к КАЖДОМУ scope отдельно, не суммарно.
...
def add_task(title: str, squad: str, status: str = "proposed",
             reason: str = "", how: str = "", goal_id: str | None = None,
    ...
    repo — опционально: SCOPE для подсчёта MAX_CONCURRENT (см. выше и
    get_active_tasks() ниже). None (по умолчанию, поведение существующих
    вызовов НЕ меняется) — задача считается "легаси/BLD" ёмкостью,
    общей для отрядов/пульса/индивидуальной инициативы. Новые продукты
    из register_project(..., repo="имя_из_TARGET_REPOS") передают сюда
    это же имя — так их задачи попадают в СВОЙ, независимый от BLD,
    счётчик, а не делят слоты с основным продуктом."""
    board = _load()
...
def get_active_tasks(scope: str | None = None) -> list[dict]:
    """Задачи в статусе in_progress прямо сейчас, ОГРАНИЧЕННЫЕ одним
    SCOPE (см. add_task(repo=...) выше) — не всей компанией сразу.

    scope=None (по умолчанию, поведение всех существующих вызовов без
    аргумента не меняется) — только задачи БЕЗ явного repo, то есть
    "легаси/BLD" пул: отряды (agents/squads.py), Company Pulse,
    индивидуальная/отрядная инициатива, board_meeting.py. Они по-прежнему
    делят один счётчик друг с другом (все реально пишут в bld-system/
    bld-panel, конфликт мержа между ними настоящий), но БОЛЬШЕ НЕ делят
    его с новыми продуктами.
    scope="<имя_репо>" — только задачи конкретного продукта, заведённого
    через register_project(..., repo="<имя_репо>") (workflows/big_projects.py)
    — своя, независимая ёмкость.
    scope="all" — буквально все in_progress задачи разом, для общего
    обзора (например, если понадобится посчитать компанию целиком).

    Перед подсчётом ОДИН РАЗ за запуск процесса подчищает зависшие
    задачи (reconcile_stale_tasks) — без этого одна убитая GitHub
    Actions job навсегда съедала бы слот, и с каждым таким зависанием
    реальная параллельность системы тихо сокращалась бы, а не росла.
    Реконсиляция сама смотрит на ВСЕ задачи независимо от scope —
    зависшая задача любого продукта размораживается одинаково.
    ...
    if scope == "all":
        return in_progress
    return [t for t in in_progress if t.get("repo") == scope]
...
def can_take_more(scope: str | None = None) -> bool:
    """True если лимит параллельных задач для этого SCOPE не исчерпан.
    scope=None — легаси/BLD пул (см. get_active_tasks() выше), как и
    было для всех существующих вызовов без аргумента. Новый продукт
    передаёт свой repo явно — его ёмкость не зависит от того, насколько
    занят BLD прямо сейчас, и наоборот."""
    return len(get_active_tasks(scope)) < MAX_CONCURRENT


def available_capacity(scope: str | None = None) -> int:
    """Сколько ещё задач можно взять в работу ПРЯМО СЕЙЧАС в этом SCOPE,
    не превышая MAX_CONCURRENT — в отличие от can_take_more() (просто
    да/нет), отдаёт конкретное число. Нужно там, где нужно решить не
    "можно ли взять ещё одну", а "скольким отрядам/людям одновременно
    можно дать задачу в этом цикле" (см. workflows/board_meeting.py —
    раньше рассинхрон между этим модулем и board_meeting.py приводил к
    тому, что задачи от совета директоров вообще не учитывались в
    лимите). scope=None — легаси/BLD пул, как и было для существующих
    вызовов без аргумента."""
    return max(0, MAX_CONCURRENT - len(get_active_tasks(scope)))
