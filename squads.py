...
ВАЖНО — эти 7 департаментов ПОСТОЯННЫЕ, но только для BLD System (её
конкретной архитектуры: evidence-to-owner pipeline, L1-L9, Telegram-бот,
React-панель). Они НЕ вся компания — Валик как CDTO может ставить цели
и на отдельные, самостоятельные продукты цифровизации стройки вне BLD
(см. context/company_context.md, workflows/big_projects.py::register_project
с repo != None). Для такого нового продукта не заводится новый
постоянный департамент — вместо этого под конкретную идею собирается
ad hoc pod из ВСЕГО пула (agents/roster.py::build_full_roster(), ~600
человек) по совпадению скилла с гранью идеи (workflows/big_projects.py
::pick_relevant_group), сколько бы человек она ни требовала — один или
много, — и распускается/трансформируется вместе с проектом, а не живёт
вечно как эти 7 департаментов.
"""
